...
def my_prompt(id_prompt=None):
    ...
    try:
        ...
        prompt = execute_query(
            "SELECT p.id, p.text, p.created_at, p.likes, u.name, u.lastname, p.likes FROM Prompt p JOIN User u ON p.id_user = u.id WHERE p.id = ?  AND p.enabled = TRUE ORDER BY p.created_at DESC",
            (id_prompt,)
        )
