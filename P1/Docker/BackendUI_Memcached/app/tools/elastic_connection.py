...
def _build_client() -> Optional[Elasticsearch]:
    # Build an Elasticsearch client instance with retries and timeouts.
    try:
        try:
...
def connect_elastic() -> Optional[Elasticsearch]:
    ...
    try: #verify connection with ping()
        if client.ping():
            logger.info("Connected to Elasticsearch")
            return client
        logger.warning("Elasticsearch ping returned False; attempting info() fallback")
    except Exception as exc:  #fallback to info() if ping fails
...
class ElasticsearchPool:
    ...
    # pool manager for Elasticsearch client instances.
    ...
    # fill the pool with client instances, retrying on failure
    def _fill_pool(self) -> bool:
        ...
        while created < self.pool_size and attempt < self._max_retries:
            ...
            if client is None:
                attempt += 1
                logger.warning("Elasticsearch client creation failed on attempt %d/%d; retrying in %ds")
...
# Execute a generic Elasticsearch query against the specified index using a pooled client. And return the hits realted with the text
#Parameters:
# index: the name of the index to search
# titlte_query: the text to search for
# size: the number of results to return (default 10)
# match_phrase: whether to use match_phrase query (default False)
# fuzziness: the fuzziness level to use (default None)
# fields: the fields to search in (default ["title"])
def execute_text_search_by_title(index, title_query, size=10, match_phrase=False, fuzziness=None, fields=None):
    ...
    if match_phrase:
        ...
        if len(fields) == 1:
            ...
            if fuzziness is not None:
                match_body["fuzziness"] = fuzziness
            query = {"multi_match": match_body} #build the multi_match query
...
# Execute a vector (kNN) search against the specified index using a pooled client. And return the hits realted with the vector
# Parameters:
# index: the name of the index to search
# query_vector: the vector to search for (list of floats or dict containing 'embedding' key)
# k: the number of nearest neighbors to return (default 10)
# vector_field: the name of the dense_vector field in the index (default "embeddings
# num_candidates: the number of candidates to consider for kNN search (default 20)
def execute_vector_query(index, query_vector, k=10, vector_field="embeddings", num_candidates=20):
    field_def = None
    # Prepare kNN body for Elasticsearch vector search
    body = {
        "size": k,
        "knn": {
            "field": vector_field,
    ...
    #using the pool to get a client
    with default_pool.get_client() as es:
        # Try kNN search first
        try:
            resp = es.search(index=index, body=body)
            hits = resp.get("hits", {}).get("hits", [])
            # Remove the vector field from _source to avoid returning large embeddings
            for h in hits:
                if isinstance(h, dict):
                    src = h.get("_source")
                    if isinstance(src, dict) and vector_field in src:
