...
val tmp2 = spark.sql("SELECT split(split(msg, ' ')[0], ':')[0] AS hour, split(split(msg, ' ')[0], ':')[1] AS minute, split(split(msg, ' ')[0], ':')[2] AS second,*  FROM tmp")
...
S hou
spark.sql("SELECT split(split(msg, ' ')[0], ':')[0] Ar, split(split(msg, ' ')[0], ':')[1] AS minute, split(split(msg, ' ')[0], ':')[2] AS second  FROM tmp").saveToEs("data")
