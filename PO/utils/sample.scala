//El codigo esta funcionanando manual hay que adaptarlo a como funcione dentro del contenedor
//Utiliza scala con spark para procesar los json
//cd utils
...
conf.set("es.net.http.auth.pass", "MnC843U3M4679dXhMMRy91uP")
...
// Lee la variable de entorno del Pod
val dataDirectoryPath = sys.env("XPATH")

// Usa la ruta del PVC para leer todos los archivos JSON
val data = spark.read.json(s"$dataDirectoryPath/*.json")

//val data = spark.read.json("/Users/caray/Documents/TEC/S2-2025/BASES DE DATOS II/PO/CODIGO/PO/utils/sample/*.json")
data.printSchema()
data.show
